...
let chartData = filteredData.map((val)=>{
    return {"section":val.section, "totalSolved":val.totalSolved};
})
chartData = chartData.map((val)=>{
    let sectionWiseSolved = chartData.reduce((next, num)=>{
        if(val.section === num.section) next+=(num.totalSolved || 0);
        return next;
    },0)
    
    return {"section":val.section, "totalSolved":sectionWiseSolved};
})
let map = new Map();
chartData.forEach((val)=>{
    if(!map.has(val.section)){
        map.set(val.section, val.totalSolved);
    }
})
...
(async function() {
  new Chart(
    document.getElementById('acquisitions'),
    {
      type: 'pie',
      data: {
        labels: arr.map(person => person.name),
        datasets: [
          {
            label: 'Acquisitions by totalSolved',
            data: arr.map(person => person.value)
          }
        ]
      }
    }
